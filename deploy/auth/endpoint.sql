...
-- alias
create type auth.signup_success as (
  message text
);
...
create type auth.jwt_pre_claims as (
  _role name,
  _id uuid
);

create type auth.jwt_claims as (
  id text,
  role text,
  email text
);
...
from
  auth.users
where
  id = current_setting('request.jwt.claim.user_id', true)::uuid;
...
begin
  ...
  using message = 'email is already registered: ' || new.email;
  return null;
end if;
  return new;
end
...
begin
  ...
  from
    auth.users
  where
    users.email = get_user_role_and_id.email
...
begin
  select
    ...
    _id
  from
    auth.get_user_role_and_id (email, password) into check_user;
  if check_user._id is null then
    raise invalid_password
    using message = 'invalid email or password';
  end if;
  select
    check_user._id as id,
    check_user._role as role,
    login.email as email into loggedin_user;
  select
    public.sign(row_to_json(r), current_setting('app.jwt_secret')) as token
  from (
    select
